...
export default function LessonPage() {
  ...
  const showAlert = (msg) => { setAlertMessage(msg); setAlertVisible(true); };
  const showConfirm = (msg, action) => { setConfirmMessage(msg); setConfirmAction(() => action); setConfirmVisible(true); };
  ...
  useEffect(() => {
    ...
    api.get(`/CourseEnrollment/courses/${courseId}/enrollment-status`)
  ...
  const handleTimeUpdate = () => { if (videoRef.current) setMaxTime(p => Math.max(p, videoRef.current.currentTime)); };
  const handleSeeking = () => { if (videoRef.current && videoRef.current.currentTime > maxTime + 0.05) videoRef.current.currentTime = maxTime; };
  const handleSeeked = () => {
    if (!videoRef.current) return;
    setTimeout(() => { if (videoRef.current.currentTime > maxTime + 0.05) videoRef.current.currentTime = maxTime; }, 50);
  };

  // Document scroll
  const onDocScroll = () => {
    const c = scrollContainerRef.current;
    if (c && c.scrollTop + c.clientHeight >= c.scrollHeight - 10) setHasScrolledDocs(true);
  };
    ...
    if (tab === "quiz") {
      ...
      if (quizAttemptCount === 3 && !hasPassed) {
        showConfirm(
          ...
          async () => {
            try {
              await api.delete(`/CourseEnrollment/courses/${courseId}/unenroll`);
              await api.post(`/CourseEnrollment/courses/${courseId}/enroll`);
              updateEnrollment();
              const res = await api.get(`/CourseEnrollment/courses/${courseId}/enrollment-status`);
      ...
      if (quizAttemptCount === 0 && !hasPassed) {
        ...
        if (watchedRatio < WATCH_THRESHOLD) { showAlert(`Bạn phải xem ít nhất ${Math.round(WATCH_THRESHOLD * 100)}% video trước khi làm Quiz.`); return; }
        if (materials.some(m => m.type === "Document") && !hasScrolledDocs) { showAlert("Bạn phải cuộn tài liệu xuống cuối để làm Quiz."); return; }
      }
    }
    setActiveTab(tab);
  };
  ...
  const handleViewDocument = async doc => {
    try {
      ...
      setDocFile(new File([buffer], `${doc.title}.docx`, { type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document" }));
      setHasScrolledDocs(false);
      setActiveTab("docs");
    } catch {
  ...
  if (loadingCourse) return <p className="text-center py-10">Đang tải khóa học…</p>;
        ...
        <div className="flex space-x-4 border-b pb-2">
          {['video', 'docs', 'quiz'].map(tab => (
            <button
              ...
              className={`${activeTab===tab?'border-b-2 border-indigo-600 text-indigo-600':'text-gray-600'} pb-2`}
          ...
          <div className="bg-white p-6 rounded-lg shadow space-y-4">
            ...
            : docs.map(doc=>(
                <div key={doc.id} className="border-b pb-4">
                  ...
                  <button onClick={()=>handleViewDocument(doc)} className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">View Document</button>
                </div>
              ))}
            {docFile&&(
              <div className="mt-6">
                <button onClick={()=>setDocFile(null)} className="mb-4 text-gray-500 hover:text-gray-800">✕ Đóng tài liệu</button>
                <div ref={scrollContainerRef} onScroll={onDocScroll} className="max-h-[60vh] overflow-auto border p-2">
              ...
              {submissions.map(sub=>(
                <li key={sub.id} className="py-3 flex justify-between items-center hover:bg-gray-50 cursor-pointer" onClick={()=>viewDetail(sub)}>
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Score: {sub.score} / {quizQuestions.length}</p>
                  </div>
      ...
      {showDetail && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowDetail(false)}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-auto p-6 relative" onClick={e => e.stopPropagation()}>
            <button className="absolute top-3 right-3 text-gray-500 hover:text-gray-800" onClick={() => setShowDetail(false)}>✕</button>
                  ...
                  <div key={qObj.questionId} className="mb-6">
                    ...
                    <ul className="space-y-1">
                      {options.map(opt => {
                        ...
                        return (
                          <li key={opt.id} className={`border-l-4 p-2 flex items-center rounded ${chosen?correct?"bg-green-50 border-green-500 text-green-800":"bg-red-50 border-red-500 text-red-800":"bg-gray-50 border-transparent text-gray-700"}`}>{chosen && <span className="mr-2">{correct?"✅":"❌"}</span>}{opt.optionText}</li>
                        );
                      })}
                    </ul>
                  </div>
      ...
      {alertVisible && (
        <div className="fixed inset-0 flex items-center justify-center z-60">
          <div className="bg-white p-4 rounded-lg shadow-lg max-w-xs text-center border border-indigo-200">
            ...
            <button onClick={() => setAlertVisible(false)} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md">OK</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-60">
          <div className="bg-white p-4 rounded-lg shadow-lg max-w-xs text-center border border-indigo-200">
            ...
            <div className="flex justify-center space-x-2">
              ...
              <button onClick={() => { confirmAction(); hideConfirm(); }} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md">OK</button>
            </div>
          </div>
        </div>
      )}
